...
| :--- | :--- |
| **Control Centralizado del Flujo de Trabajo:** Proporciona una visibilidad completa y explícita de los procesos de negocio complejos, facilitando su comprensión y mantenimiento. | **Punto Único de Fallo (SPOF):** El orquestador se convierte en un componente crítico; su indisponibilidad detiene todos los flujos de trabajo. *(Mitigación: se implementará una configuración de alta disponibilidad para Apache Airflow).* |
| **Manejo de Errores Simplificado:** La lógica de reintentos, compensación y alertas se centraliza en el orquestador, en lugar de distribuirse entre múltiples servicios. | **Acoplamiento al Orquestador:** Los servicios, aunque se desacoplan entre sí, desarrollan un acoplamiento con la API del orquestador. *(Mitigación: definición de interfaces agnósticas y uso de patrones de adaptador para aislar los servicios).* |
| **Alta Recuperabilidad y Auditabilidad:** Facilita el reinicio de flujos desde un punto de fallo específico y simplifica enormemente el seguimiento de la traza de ejecución para fines de auditoría. | **Potencial Cuello de Botella:** Un alto volumen de flujos de trabajo concurrentes podría sobrecargar el orquestador. *(Mitigación: escalado proactivo de la arquitectura de Airflow y monitoreo continuo del rendimiento).* |
| **Desacoplamiento de la Lógica de Flujo:** Los servicios individuales se centran únicamente en su tarea específica y no necesitan conocer el contexto del flujo de trabajo completo, lo que promueve su reutilización. | |
